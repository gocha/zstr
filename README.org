...
For output access (compression), the buffer size, the compression level and the window bits (zlib compression or gzip compression) can be specified by this API.
